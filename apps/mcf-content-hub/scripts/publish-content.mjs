...
const media=await put(mediaPath,bytes,{access:'public',allowOverwrite:true,token,contentType:'video/mp4'});
let manifest=[];const found=await list({prefix:manifestPath,limit:1,token});const current=found.blobs.find((blob)=>blob.pathname===manifestPath);if(current){const response=await fetch(current.url,{cache:'no-store'});if(response.ok) manifest=await response.json();}
...
manifest=[item,...manifest.filter((entry)=>entry.slug!==item.slug)];await put(manifestPath,JSON.stringify(manifest,null,2),{access:'public',allowOverwrite:true,token,contentType:'application/json'});
