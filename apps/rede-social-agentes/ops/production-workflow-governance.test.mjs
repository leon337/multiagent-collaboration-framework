...
test('production workflow has no automatic triggers', async () => {
...
test('dispatch cannot manufacture production authorization', async () => {
  const workflow = await productionWorkflow();
...
test('provider config comes only from protected secrets', async () => {
  const workflow = await productionWorkflow();
...
test('promotion exposes explicit governed outcomes', async () => {
  const workflow = await productionWorkflow();
