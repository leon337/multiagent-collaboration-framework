    ...
    [
      {
        ...
        x: 0,
        width: 1360,
        title: 'GROK BOT - Branch · Saudação inicial - Brave',
      },
...
test('builds an OCR band below browser chrome while preserving window bounds', () => {
  assert.deepEqual(ocrBand({ x: 1356, y: 0, width: 680, height: 714 }), {
    x: 1356,
    y: 72,
    width: 680,
    height: 220,
  ...
  assert.deepEqual(ocrBand({ x: 0, y: 0, width: 500, height: 120 }), {
    x: 0,
    y: 60,
    width: 500,
    height: 60,
  ...
  const result = validateAuditResult(
    {
      ...
      artifacts: {
        ...
        verification: '/tmp/verification.png',
      },
      elapsedMs: 3448,
      openVerified: true,
      criticalFailures: [],
  ...
  const result = validateAuditResult(
    {
      ...
      surfaces: [
        { windowId: '0x1', title: 'left', x: 0, y: 0, width: 100, height: 100, label: 'visible' },
      ],
      artifacts: {
        raw: '/tmp/raw.png',
        annotated: '/tmp/annotated.png',
