...
const schema = z
  .object({
    ...
    BOOTSTRAP_INITIAL_HUMAN_EMAIL: z.string().trim().email().optional(),
    BOOTSTRAP_INITIAL_REGISTRATION_TOKEN: z.string().min(32).optional(),
  })
  .superRefine((config, context) => {
    const emailSet = Boolean(config.BOOTSTRAP_INITIAL_HUMAN_EMAIL);
    const tokenSet = Boolean(config.BOOTSTRAP_INITIAL_REGISTRATION_TOKEN);
    if (emailSet !== tokenSet) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['BOOTSTRAP_INITIAL_HUMAN_EMAIL'],
        message: 'Initial-human email and registration token must be configured together.',
      });
    }
  });
