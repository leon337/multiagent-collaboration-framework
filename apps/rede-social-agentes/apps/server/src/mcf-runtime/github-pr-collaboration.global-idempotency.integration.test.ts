...
function missionContract(title: string) {
  return JSON.stringify({
    title,
    objective:
      'Serialize C2 mutations globally while allowing safe retry after definite pre-write failure.',
    expectedOutcome:
      'Only active or externally-consumed logical mutations keep the global binding.',
...
describe('C2 global idempotency serialization', () => {
  ...
  it(
    'releases a definite pre-write FAILED binding so the same logical operation can retry',
    async () => {
      ...
      try {
        ...
        const changedPayload =
          outcomes[0]?.status === 'fulfilled'
            ? request(missionA, randomUUID(), {
                comment_body: 'different payload with reused key',
              })
            : request(missionB, randomUUID(), {
                comment_body: 'different payload with reused key',
              });
