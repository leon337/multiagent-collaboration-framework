...
const SAFE_REF = /^(?!\/)(?!.*\.\.)(?!.*(?:^|\/)\.)(?!.*[~^:?*\[\\\s])(?!.*\/$)(?!.*\.lock$)[A-Za-z0-9._\/-]{1,200}$/u;
...
function requireString(inputs: Record<string, unknown>, key: string): string {
  ...
  if (typeof value !== 'string' || value.length === 0 || value !== value.trim()) {
    throw new ExternalActionAdapterError('INVALID_CONTEXT', `${key} must be a non-empty trimmed string`, false);
  }
  return value;
}
...
function exactSha(value: string, label: string): string {
  ...
  if (!SHA_40.test(normalized)) {
    throw new ExternalActionAdapterError('INVALID_CONTEXT', `${label} must be an exact 40-character SHA`, false);
  }
  return normalized;
}
...
function safeRef(value: string, label: string): string {
  if (!SAFE_REF.test(value) || value.startsWith('refs/') || value.endsWith('.')) {
    throw new ExternalActionAdapterError('INVALID_CONTEXT', `${label} is not a supported GitHub branch ref`, false);
  }
  return value;
}
...
function resolveTarget(request: ExternalActionRequest): BranchPrTarget {
  if (!request.context) {
    throw new ExternalActionAdapterError('INVALID_CONTEXT', 'GitHub branch/PR write requires governed execution context', false);
  }
  if (!REPOSITORY.test(request.tool.resource)) {
    throw new ExternalActionAdapterError('UNSUPPORTED_TARGET', 'GitHub branch/PR write requires canonical owner/repository resource', false);
  }

  const repository = requireString(request.inputs, 'repository');
  if (!REPOSITORY.test(repository) || repository.toLowerCase() !== request.tool.resource.toLowerCase()) {
    throw new ExternalActionAdapterError('INVALID_CONTEXT', 'repository input must exactly match the canonical tool resource', false);
  }
  ...
  if (branchRef.toLowerCase() === 'main' || branchRef === baseRef) {
    throw new ExternalActionAdapterError('INVALID_CONTEXT', 'branch_ref must be a new non-main branch distinct from base_branch', false);
  }
  const idempotencyKey = requireString(request.inputs, 'idempotency_key');
  if (!IDEMPOTENCY_KEY.test(idempotencyKey)) {
    throw new ExternalActionAdapterError('INVALID_CONTEXT', 'idempotency_key must be 16-128 safe characters', false);
  }
...
function assertRef(response: GitHubRefResponse, expectedRef: string, expectedSha: string): void {
  if (response.ref !== `refs/heads/${expectedRef}` || exactSha(response.object?.sha ?? '', 'provider ref SHA') !== expectedSha) {
    throw new ExternalActionAdapterError('RESERVATION_CONFLICT', `GitHub branch ${expectedRef} does not match the expected SHA`, false);
  }
}
...
function assertPull(pull: GitHubPullResponse, target: BranchPrTarget): void {
  if (
    ...
    throw new ExternalActionAdapterError('RESERVATION_CONFLICT', 'GitHub pull request does not match the controlled MCF write', false);
  }
}
...
export class GitHubBranchPrClient {
  ...
  constructor(
    ...
    private readonly token: string | undefined = process.env.MCF_GITHUB_TOKEN ?? process.env.GITHUB_TOKEN,
  ) {}
  ...
  async requestJson<T>(
    ...
    if (!path.startsWith('/repos/') || path.includes('://') || !['GET', 'POST'].includes(method)) {
      throw new ExternalActionAdapterError('UNSUPPORTED_TARGET', 'GitHub branch/PR client rejected an unsupported request path or method', false);
    }
    this.requestCount += 1;
    if (this.requestCount > MAX_REQUESTS) {
      throw new ExternalActionAdapterError('INVALID_RESPONSE', 'GitHub branch/PR request budget exceeded', false);
    }
    const remaining = deadlineAt - Date.now();
    if (remaining <= 0) {
      throw new ExternalActionAdapterError('ADAPTER_TIMEOUT', 'GitHub branch/PR adapter exceeded its execution deadline', true);
    }
    ...
    try {
      ...
      try {
        ...
        throw new ExternalActionAdapterError('NETWORK_FAILURE', error instanceof Error ? error.message : 'GitHub branch/PR network request failed', true);
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 || (response.status === 403 && (remainingRate === '0' || retryAfter !== null))) {
          throw new ExternalActionAdapterError('RATE_LIMITED', 'GitHub API rate limit was reached', true, response.status);
        }
        if (response.status === 401 || response.status === 403) {
          throw new ExternalActionAdapterError('AUTHENTICATION_REQUIRED', 'GitHub authentication or write permission is required', false, response.status);
        }
        if (response.status === 404) {
          throw new ExternalActionAdapterError('TARGET_NOT_FOUND', 'GitHub branch/PR target was not found', false, 404);
        }
        if (response.status === 409 || response.status === 422) {
          throw new ExternalActionAdapterError('RESERVATION_CONFLICT', `GitHub rejected the controlled write with HTTP ${response.status}`, false, response.status);
        }
        throw new ExternalActionAdapterError('INVALID_RESPONSE', `GitHub API returned HTTP ${response.status}`, response.status >= 500, response.status);
      }
      try {
        ...
        throw new ExternalActionAdapterError('INVALID_RESPONSE', 'GitHub API returned invalid JSON', false, response.status);
      }
    } finally {
...
export class GitHubBranchPullRequestAdapter implements ExternalActionAdapter {
  ...
  private async getBranch(target: BranchPrTarget, deadlineAt: number): Promise<GitHubRefResponse | null> {
  ...
  private async findPull(target: BranchPrTarget, deadlineAt: number): Promise<GitHubPullResponse | null> {
    ...
    if (!Array.isArray(pulls)) {
      throw new ExternalActionAdapterError('INVALID_RESPONSE', 'GitHub pull lookup did not return an array', false);
    }
    ...
    if (compatible.length > 1) {
      throw new ExternalActionAdapterError('RESERVATION_CONFLICT', 'Multiple GitHub pull requests match the same MCF idempotency key', false);
    }
    if (compatible[0]) return compatible[0];
    if (pulls.length > 0) {
      throw new ExternalActionAdapterError('RESERVATION_CONFLICT', 'An incompatible pull request already exists for the requested branch/base', false);
    }
    return null;
  }

  private receipt(request: ExternalActionRequest, target: BranchPrTarget, pull: GitHubPullResponse): McfToolReceipt {
    assertPull(pull, target);
  ...
  async execute(request: ExternalActionRequest): Promise<McfToolReceipt> {
    ...
    if (!base) throw new ExternalActionAdapterError('TARGET_NOT_FOUND', 'Base branch was not found', false);
    ...
    if (!commit || exactSha(commit.sha, 'provider commit SHA') !== target.headSha) {
      throw new ExternalActionAdapterError('INVALID_RESPONSE', 'GitHub did not verify the exact requested head SHA', false);
    }

    let branch = await this.getBranch(target, deadlineAt);
    if (branch) {
      ...
      try {
        ...
        branch = await this.getBranch(target, deadlineAt);
        if (!branch) throw error;
      }
      branch = await this.getBranch(target, deadlineAt);
      if (!branch) {
        throw new ExternalActionAdapterError('INVALID_RESPONSE', 'GitHub branch creation could not be verified by read-back', true);
      }
      assertRef(branch, target.branchRef, target.headSha);
    }

    let pull = await this.findPull(target, deadlineAt);
    if (!pull) {
      try {
        await this.client.requestJson<GitHubPullResponse>(
          ...
          deadlineAt,
          { title: target.title, head: target.branchRef, base: target.baseRef, body: target.body },
        );
      } catch (error) {
        if (!(error instanceof ExternalActionAdapterError) || !error.retryable) throw error;
        pull = await this.findPull(target, deadlineAt);
        if (!pull) throw error;
      }
      pull = await this.findPull(target, deadlineAt);
      if (!pull) {
        throw new ExternalActionAdapterError('INVALID_RESPONSE', 'GitHub pull request creation could not be verified by read-back', true);
      }
    }
    assertPull(pull, target);
    return this.receipt(request, target, pull);
  }
}
