...
function validContract(): CreateMcfMissionRequest['contract'] {
  return {
    title: 'Governed v1.1 production promotion',
    objective: 'Promote one exact qualified MCF revision through the governed production boundary.',
...
describe('MissionRuntimeController v1.1 admission boundary', () => {
  it('preserves the v1.1 extension through HTTP body parsing', async () => {
    ...
    await controller.createMission({ contract: validContract() }, {
      id: 'correlation-v11-controller-test',
    } as never);
  ...
  it('rejects an incomplete v1.1 extension before Mission Runtime execution', async () => {
    ...
    await expect(
      controller.createMission(
        {
          contract: {
            title: 'Incomplete v1.1 mission',
            objective: 'Attempt to create a v1.1 mission without its canonical project context.',
            expectedOutcome: 'The HTTP boundary rejects the incomplete mission.',
            scope: ['admission'],
            outOfScope: [],
            acceptanceCriteria: ['runtime is not called'],
            riskClass: 'C',
            selectedAgents: ['LÉO'],
            selectedSkills: ['MCF-DEPLOY-VALIDATE'],
            sourceOfTruth: ['leon337/multiagent-collaboration-framework'],
        ...
        { id: 'correlation-v11-incomplete' } as never,
      ),
    ).rejects.toBeInstanceOf(BadRequestException);
    expect(createMission).not.toHaveBeenCalled();
  });

  it('maps a verified-context rejection to HTTP 422 instead of an internal error', async () => {
    const createMission = vi.fn(async () => {
      throw new MissionV11ContextError(
        'ALIGNED_PIP_PAIR_INVALID',
        'canonical PIP and alignment receipt do not verify',
      );
    });
    const controller = new MissionRuntimeController({
      createMission,
    ...
    await expect(
      controller.createMission({ contract: validContract() }, {
        id: 'correlation-v11-invalid-context',
      } as never),
