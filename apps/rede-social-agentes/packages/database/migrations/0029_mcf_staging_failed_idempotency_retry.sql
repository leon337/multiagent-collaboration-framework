-- MCF-RUNTIME-006-GATE-D independent-review remediation.
--
-- Preserve FAILED attempts as fingerprint tombstones, but explicitly allow a
-- compatible retry to reclaim the global idempotency scope for the two write
-- adapters whose dispatcher contract treats FAILED as definitively not applied.
-- UNKNOWN remains bound and is never released by this function.
...
begin
  ...
  -- FAILED is definitively not applied for these dispatcher/adapter contracts.
  -- Keep the failed row bound as a tombstone until a replacement proves it is
  -- the same canonical request. An incompatible retry remains blocked by the
  -- persistent unique scope index. UNKNOWN is intentionally not handled here.
  if holder."status" = 'FAILED' then
    if compatible_fingerprint
       and (
         holder."adapter_id" = 'github-pr-collaboration-write-v1'
         or (
           holder."adapter_id" = 'github-actions-staging-deploy-v1'
           and holder."provider" = 'github'
           and holder."operation" = 'deploy-staging'
         )
       ) then
