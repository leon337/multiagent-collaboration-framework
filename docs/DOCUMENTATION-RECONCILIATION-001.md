...
## 2. Baseline pós-stable verificada
...
GitHub live confirma `v1.0.0` no SHA exato da RC3; Release `MCF v1.0.0` é não-draft, não-prerelease e `latest`. Issue #131 está `closed/completed`; PR #133 está `closed/unmerged`.
...
| Documento/área | Afirmação antiga ou risco | Evidência pós-stable | Correção | Classificação |
|---|---|---|---|---|
| `README.md` | stable `NOT_PUBLISHED`, HUMAN_GATE `NOT_APPROVED` | stable publicada; gate consumido/protegido | snapshot atual e tabela de releases atualizados | `SUPERSEDED` → `CURRENT_IMPLEMENTED` |
...
O CHANGELOG permanece estruturado por milestones, não por commit dump. A reconciliação pós-stable adiciona `v1.0.0` como marco publicado em 2026-08-14 e reclassifica o wording pré-publicação como contexto histórico.

## 8. README e índices

O README e os índices atuais expõem:

- runtime executável e sua localização;
- capacidades comprovadas;
- `main`, produção e stable publicada;
...
`docs/runtime/README.md` representa o lineage pós-stable. O plano detalhado do RUNTIME-006 permanece `HISTORICAL`; seus SHAs e estados antigos não são reescritos.
