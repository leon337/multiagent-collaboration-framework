...
const CharacterCycle=()=>{
  ...
  const slice=Math.max(1,Math.ceil(pedagogySync.scenes.characters.durationFrames/variants.length));
...
export const PedagogyCharactersShowcase=()=> <AbsoluteFill>
  ...
  <Scene id="recall"><ActiveRecall question="Quem executa a ação?" answer="O runtime autorizado." revealAfterFrame={Math.max(45,Math.floor(pedagogySync.scenes.recall.durationFrames*.55))}/></Scene>
  <Scene id="quiz"><Quiz question="Qual recurso força uma decisão antes da resposta?" options={["Title","Active Recall","Background"]} correctIndex={1} revealAfterFrame={Math.max(50,Math.floor(pedagogySync.scenes.quiz.durationFrames*.6))}/></Scene>
