...
export type TechnicalLessonSpec={
  ...
  theme:{id:string;background:string;accent:string};assets:string[];timings:{fps:number};
};
export const getTechnicalLessonDuration=(spec:TechnicalLessonSpec)=>spec.scenes.reduce((sum,scene)=>sum+scene.durationFrames,0);
