...
import {alignSceneLayout,normalizeSceneLayout,nudgeSceneLayout,snapSceneLayout} from '../src/authoring/VisualAuthoring';
...
describe('Video Lab V3 authoring operations',()=>{
 ...
 it('aligns and nudges scene layout deterministically',()=>{const base=normalizeSceneLayout({x:.2,y:.2,scale:1,rotationDeg:0,snap:false});expect(alignSceneLayout(base,'center-x').x).toBe(0);expect(alignSceneLayout(base,'top').y).toBe(-.35);expect(nudgeSceneLayout(base,.01,-.01)).toMatchObject({x:.21,y:.19});});
