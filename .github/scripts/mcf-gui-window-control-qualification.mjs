...
function validateTrace(trace) {
  const errors = [];
  const requiredTop = [
    'trace_version', 'mission_id', 'classification', 'predecessor', 'successor',
    'handoff', 'window_control', 'input_evidence', 'observability',
  ];
  for (const key of requiredTop) if (!(key in trace)) errors.push(`missing ${key}`);
  if (errors.length) return errors;

  if (trace.trace_version !== 'mcf_gui_window_succession_trace/v1') errors.push('unexpected trace_version');
  if (trace.classification !== 'MAINTAIN_WITH_GAP') errors.push('classification must be MAINTAIN_WITH_GAP');
  ...
  if (p.surface_preserved_through_equivalence_and_handoff !== true) errors.push('predecessor surface must be preserved through equivalence and handoff');
  ...
  if (!['X11_SYNTHETIC_EVENT', 'DEVICE_LEVEL_INPUT_EVENT', 'PHYSICAL_INPUT_EVENT'].includes(i.mechanism)) errors.push('input mechanism must be truthful and enumerated');
  if (i.claimed_equivalent_to_physical_input === true && i.mechanism === 'X11_SYNTHETIC_EVENT') errors.push('X11 synthetic event cannot be claimed equivalent to physical input');
...
requireTokens('protocol', protocolText, [
  ...
  'OPEN_NEW_WINDOW', 'OPEN_NEW_CHAT', 'PREDECESSOR_SURFACE_PRESERVED',
  'SUCCESSION_EQUIVALENCE', 'EXPLICIT_HANDOFF', 'PREDECESSOR_CLOSE',
  'VISUAL_ASSERTION', 'X11_SYNTHETIC_EVENT != DEVICE_LEVEL_INPUT_EVENT',
  'MONITOR_AWARE', 'HUMANO NO CONTROLE',
]);
...
if (schema) {
  if (schema.$id !== 'https://mcf.local/schemas/mcf-gui-window-succession-trace-v1.schema.json') fail('schema $id mismatch');
  else pass('schema $id');
